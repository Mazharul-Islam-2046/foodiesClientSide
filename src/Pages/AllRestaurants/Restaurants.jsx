...
const AllRestaurants = () => {
    ...
    const {
        ...
        queryKey: ['fillteredRestaurants', filters, searchName],
        queryFn: async ({ pageParam = 1 }) => {
            try {
                const response = await restaurantApi.filterRestaurants(pageParam, 10, restFilters, sortBy);
        ...
        getNextPageParam: (lastPage, allPages) => {
            ...
            if (lastPage?.restaurants?.length === 0) return undefined;
    ...
    const hasData = filteredRestaurants?.pages && filteredRestaurants?.pages[0].length > 0;
